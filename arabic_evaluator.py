...
#!/usr/bin/env python3
"""
Arabic Multiple Choice Question Evaluator for MoE-PEFT

This evaluator is specifically designed for Arabic culture understanding tasks
using the PalmX dataset and Fanar model.
"""
...
class MCQProcessor:
    ...
    """
    def __init__(self, model_name: str, adapter_path: str, device: Optional[str] = None,
                 prompt_template: str = DEFAULT_PROMPT_TEMPLATE,
                 choice_prefixes: Optional[List[str]] = None,
                 load_16bit: bool = True, load_8bit: bool = False, load_4bit: bool = False):
        ...
        try:
            ...
            if adapter_path and os.path.exists(adapter_path):
                ...
                self.adapter_name = self.model.init_adapter(
                    moe_peft.AdapterConfig(adapter_name="default")
                )
                print("Using default adapter (no trained adapter provided)")
    ...
    def process_batch(self, batch_docs):
        """
        Processes a batch of MCQ documents with improved error handling.
        """
        if not batch_docs:
            return [], [], []
        ...
        for doc in batch_docs:
            try:
                ...
                print(f"Warning: Failed to process document {doc.get('id', 'unknown')}: {e}")
                doc_choice_counts.append(0)
                continue
        ...
        for b_idx in range(len(flat_full_sequences)):
            try:
                ...
                print(f"Warning: Error processing sequence {b_idx}: {e}")
                flat_choice_log_likelihoods.append(-float('inf'))
        ...
        for doc_idx, num_choices in enumerate(doc_choice_counts):
            if num_choices == 0:
                all_predicted_labels.append("N/A")
                all_probabilities.append([])
                all_scores_grouped.append([])
                continue
    ...
    def calculate_accuracy(self, predicted_labels: List[str], ground_truth_labels: List[str]) -> float:
...
def main(
    base_model: str = "QCRI/Fanar-1-9B-Instruct",
    adapter_path: str = "/export/home/hhassan/CultranAI-mixlora/outputs/arabic_training_20250818_130728/arabic_fanar_mixlora_dynamic",
    dataset_name: str = "UBC-NLP/palmx_2025_subtask1_culture",
    dataset_split: str = "test",
    batch_size: int = 8,
    output_dir: str = "/export/home/hhassan/palmx/predictions",
    ...
    """
    Evaluate Arabic MCQ model on culture understanding tasks.
    
    This evaluator is specifically designed for Arabic Multiple Choice Questions
    using the MoE-PEFT framework with Fanar models.
    
    Args:
        base_model: HuggingFace model name or path (default: QCRI/Fanar-1-9B-Instruct)
        adapter_path: Path to trained adapter weights
        dataset_name: HuggingFace dataset name (default: UBC-NLP/palmx_2025_subtask1_culture)
        dataset_split: Dataset split to evaluate ('test', 'dev', 'train')
        batch_size: Batch size for evaluation (adjust based on GPU memory)
        output_dir: Directory to save prediction results
        load_16bit: Use 16-bit precision (recommended)
        load_8bit: Use 8-bit quantization (for memory savings)
        load_4bit: Use 4-bit quantization (for maximum memory savings)
        device: Device to use (auto-detected if None)
        max_seq_len: Maximum sequence length for tokenization
        
    Example usage:
        # Basic evaluation with defaults
        python arabic_evaluator.py
        
        # Custom adapter and dataset
        python arabic_evaluator.py \\
        ...
        python arabic_evaluator.py --load_4bit --batch_size 16
    """
    ...
    try:
        for i in range(0, len(dev_data), batch_size):
            ...
            if i == 0:
                ...
                for doc_idx, doc in enumerate(batch_docs):
                    ...
                    print(f"🔮 Predicted: '{pred}' | ✅ Truth: '{truth}' | {'✅ CORRECT' if pred == truth else '❌ INCORRECT'}")
                print("\n" + "=" * 60)
        ...
        # Generate output filename based on adapter path
        adapter_name = os.path.basename(adapter_path) if adapter_path else "no_adapter"
        ...
        predictions_file = os.path.join(output_dir, f'{adapter_name}_{timestamp}.csv')
        ...
        if wrong_questions:
            ...
            wrong_file_path = os.path.join(output_dir, f'wrong_{adapter_name}_{timestamp}.csv')
            wrong_df.to_csv(wrong_file_path, index=False)
            print(f"📋 Wrong questions ({len(wrong_questions)}) saved to: {wrong_file_path}")
        else:
            print("🎉 No wrong predictions to save - perfect score!")
